...
from rapp.simulations import simulator as sim
...
def run(folder, show=False):
    ...
    cycles = 0.15
    fc = sim.samples_per_cycle(step=0.5)
    noise = (0, 0.04)
    mu, sigma = noise
    bits = 6
    A = 1.7
    ...
    # Pure signal
    xs, ys = sim.harmonic(A=A, cycles=cycles, fc=fc, all_positive=True)
    xs = np.rad2deg(xs)
    ...
    # Noisy signal
    xs, ys = sim.harmonic(A=A, cycles=cycles, fc=fc, noise=noise, all_positive=True)
    xs = np.rad2deg(xs)
    ...
    # Quantized signal
    xs, ys = sim.harmonic(A=A, cycles=cycles, fc=fc, noise=noise, bits=bits, all_positive=True)
    xs = np.rad2deg(xs)
